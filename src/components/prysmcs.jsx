...
import { TimeSeriesChartManager } from './time-series-chart/TimeSeriesChartManager';
...
function CustomizationProvider({ children }) {
  ...
  const addTab = (tabData) => {
    ...
    let counter = 1;
    while (customization.navigation.tabs.some(tab => tab.id === uniqueId)) {
      uniqueId = `${sanitizedId}-${counter}`;
      counter++;
    }

    const maxOrder = Math.max(...customization.navigation.tabs.map(t => t.order), -1);
  ...
  const getTabSectionCount = (tabId) => {
  ...
  return (
    <CustomizationContext.Provider value={{
      ...
      getFormSchema: () => {
        ...
        // (i.e., sections haven't been explicitly managed by the user)
        // This prevents deleted sections from being re-added
        const deletedSections = customization.deletedSectionIds || [];
        const existingSectionIds = new Set(sectionsWithLinks.map(s => s.id));

        // Only add sections that:
        // 1. Don't exist in current schema
        // 2. Haven't been explicitly deleted by the user
        // 3. Are in the DEFAULT_FORM_SCHEMA
        const newSections = DEFAULT_FORM_SCHEMA.sections.filter(
          defaultSection =>
            !existingSectionIds.has(defaultSection.id) &&
            !deletedSections.includes(defaultSection.id)
      ...
      updateDeletedSectionIds: (deletedIds) => {
        setCustomization(prev => ({
          ...prev,
...
function Topbar({ onExport, onPresent, selectedClientId, onClientChange, selectedMonth, onMonthChange, hasUnsavedChanges, canGoBack, onGoBack, onAddClient, onDeleteClient, clients, onNavigateToAction, onToggleMobileMenu }) {
...
function CustomTabPage({ tabId, tabLabel, data, sectionVisibility = {} }) {
...
function PageSummarySection({ section, clientId, pageId }) {
  ...
  if (summaryItems.length === 0) {
    console.log('[PageSummarySection] No items to display for section:', section.id);
    return (
      <div className="summary-card">
        <h3 className="summary-title">{section.title}</h3>
        {section.subtitle && <p className="summary-subtitle">{section.subtitle}</p>}
        <p style={{ color: 'var(--slate-500)', fontSize: '14px', fontStyle: 'italic' }}>
          No summary items configured yet.
        </p>
      </div>
    );
  }
...
function OverviewPage({ data, sectionVisibility = {}, brandColor: propBrandColor, clientId = 'demo-client' }) {
  const { customization, getFormSchema, FIELD_TYPES } = useCustomization();
  const brandColor = propBrandColor || customization.branding.primaryColor;
  const formSchema = getFormSchema();
  const pageId = 'overview';

  // Find all page_summary sections for this page
  const pageSummarySections = formSchema.sections
    .filter(s => s.sectionType === 'page_summary' && s.linkedTabId === pageId && s.enabled !== false)
    .sort((a, b) => (a.order || 0) - (b.order || 0));

  console.log('[OverviewPage] Page summary sections:', pageSummarySections.map(s => ({
    id: s.id,
    title: s.title,
    linkedTabId: s.linkedTabId,
    enabled: s.enabled,
    sectionType: s.sectionType
  })));
  console.log('[OverviewPage] clientId:', clientId, 'pageId:', pageId);
  ...
  console.log('[OverviewPage] visibility:', visibility, 'coreMetricsEnabled:', coreMetricsEnabled);
  ...
  return (
    <div className="dashboard-page">
      ...
      {/* Render custom page_summary sections */}
      {pageSummarySections.map(section => {
        console.log('[OverviewPage] Rendering PageSummarySection:', section.id, section.title);
        return (
          <PageSummarySection
            key={section.id}
            section={section}
...
function EnrollmentPage({ data, sectionVisibility = {}, brandColor: propBrandColor, clientId = 'demo-client' }) {
  ...
  // Get page summary sections for this tab
  const pageSummarySections = formSchema.sections
    .filter(s => s.sectionType === 'page_summary' && s.linkedTabId === pageId && s.enabled !== false)
    .sort((a, b) => (a.order || 0) - (b.order || 0));
...
function OutcomesPage({ data, sectionVisibility = {}, clientId = 'demo-client' }) {
  ...
  const formatValue = (field, value) => {
    ...
    // Special case for avgResponseHours
...
function StoriesPage({ stories, sectionVisibility = {}, healthData = null, clientId = 'demo-client' }) {
  const { getFormSchema } = useCustomization();
  const formSchema = getFormSchema();
  ...
  const hasContent = stories.length > 0 || hasWinsAndAchievements;
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
        ...
        {stories.length > 0 ? stories.map((story, i) => (
          <div key={story.id || i} className="story-card">
            ...
            <div className="story-meta">— {story.patientType || 'Anonymous'}</div>
          </div>
        )) : !hasWinsAndAchievements ? (
...
function FormSectionRenderer({
  ...
  const [tempWidthUnits, setTempWidthUnits] = useState(section.widthUnits || 12);

  useEffect(() => {
    setTempWidthUnits(section.widthUnits || 12);
  }, [section.widthUnits]);
  ...
  const displayTitle = linkedTab?.label || section.title;
  const displayIcon = linkedTab?.icon || section.icon;
  ...
  const handleUpdateWidth = (newWidth) => {
    onSectionUpdate(section.id, { widthUnits: newWidth });
  };
  
  const widthUnits = section.widthUnits || 12;

  return (
    <div
      className={`form-section schema-section ${!section.enabled ? 'disabled' : ''} grid-item`}
      ...
      {section.enabled ? (
        <>
          {section.sectionType === 'page_summary' ? (
            <>
              {console.log('[FormSectionRenderer] Passing to PageSummaryManager:', {
                sectionId: section.id,
                sectionLinkedTabId: section.linkedTabId,
          ...
          ) : section.sectionType === 'time_series_chart' ? (
            <>
              {console.log('[FormSectionRenderer] Passing to TimeSeriesChartManager:', {
                sectionId: section.id,
                sectionLinkedTabId: section.linkedTabId,
                receivedPageId: pageId,
                clientId,
          ...
          {showEditControls && section.sectionType !== 'page_summary' && section.sectionType !== 'time_series_chart' && (
            <div style={{ marginTop: '16px', padding: '16px', background: 'var(--slate-50)', borderRadius: '8px', border: '1px solid var(--slate-200)' }}>
          ...
          {showEditControls && section.canAddFields !== false && section.sectionType !== 'page_summary' && section.sectionType !== 'time_series_chart' && (
...
function TabbedFormRenderer({
  ...
  const enabledTabs = tabs.filter(tab => tab.enabled).sort((a, b) => (a.order || 0) - (b.order || 0));
...
function SchemaFormRenderer({
  ...
  const {
    ...
    addFormSection: globalAddFormSection,
  ...
  const addFormSection = localHandlers?.addFormSection || globalAddFormSection;
  ...
  const [newSectionData, setNewSectionData] = useState({
    ...
    subtitle: '',
    icon: 'FileText',
    linkedTabId: filterByTab || 'overview',
  ...
  const handleAddSection = () => {
    if (!newSectionData.id || !newSectionData.title || !newSectionData.linkedTabId) return;

    const newSection = createFormSection(newSectionData.id, {
      title: newSectionData.title,
      subtitle: newSectionData.subtitle,
      icon: newSectionData.icon,
      linkedTabId: newSectionData.linkedTabId,
      fields: [],
      sectionType: newSectionData.sectionType,
      summaryMode: newSectionData.summaryMode,
      layoutStyle: newSectionData.layoutStyle,
      maxItems: newSectionData.maxItems,
      widthUnits: newSectionData.widthUnits,
    });
  ...
  return (
    <div className="schema-form grid-container">
            ...
            <div style={{ padding: '20px' }}>
              <div className="form-grid" style={{ gridTemplateColumns: '1fr 1fr' }}>
                <div className="form-group" style={{ gridColumn: '1 / -1' }}>
                  ...
                  <select
                    ...
                    onChange={(e) => setNewSectionData(prev => ({
                      ...
                      title: e.target.value === 'page_summary' ? 'Page Summary' : e.target.value === 'time_series_chart' ? 'Historical Trend' : prev.title
                    }))}
                  >
                    <option value="standard">Standard Section (with fields)</option>
                    <option value="page_summary">Page Summary (metrics overview)</option>
                    <option value="time_series_chart">Time Series Chart (historical data)</option>
                  </select>
                </div>

                {newSectionData.sectionType === 'page_summary' && (
                  <>
                    <div className="form-group" style={{ gridColumn: '1 / -1' }}>
                      <label className="form-label">Data Source</label>
                      <select
                        className="form-input"
                        value={newSectionData.summaryMode}
                        onChange={(e) => setNewSectionData(prev => ({ ...prev, summaryMode: e.target.value }))}
                      >
                        <option value="manual">Manual Entry</option>
                        <option value="ai">AI-Powered (requires configuration)</option>
                      </select>
                      <p style={{ fontSize: '12px', color: 'var(--slate-500)', marginTop: '4px' }}>
                        {newSectionData.summaryMode === 'manual'
                          ? 'You will manually create and edit summary items'
                          : 'AI will generate summary items based on your data'}
                      </p>
                    </div>
                    ...
                    <div className="form-group">
                      ...
                      <select
                        className="form-input"
                        value={newSectionData.layoutStyle}
                        onChange={(e) => setNewSectionData(prev => ({ ...prev, layoutStyle: e.target.value }))}
                      >
                    ...
                    <div className="form-group">
                      ...
                      <select
                        className="form-input"
                        value={newSectionData.maxItems}
                        onChange={(e) => setNewSectionData(prev => ({ ...prev, maxItems: parseInt(e.target.value) }))}
                      >
                ...
                <div className="form-group">
                  ...
                  <select
                    className="form-input"
                    value={newSectionData.linkedTabId}
                    onChange={(e) => setNewSectionData(prev => ({ ...prev, linkedTabId: e.target.value }))}
                  >
                    {customization.navigation.tabs
                      .filter(tab => tab.enabled)
                      .sort((a, b) => a.order - b.order)
              ...
              <div style={{ marginTop: '16px', padding: '16px', background: 'var(--slate-50)', borderRadius: '8px' }}>
                <label style={{ display: 'block', fontSize: '13px', fontWeight: 500, color: 'var(--slate-700)', marginBottom: '8px' }}>
                  Section Width (Grid Columns)
                </label>
                <div style={{ display: 'flex', gap: '8px', marginBottom: '8px', flexWrap: 'wrap' }}>
                  <button
                    type="button"
                    onClick={() => setNewSectionData(prev => ({ ...prev, widthUnits: 3 }))}
                    style={{
                      padding: '6px 12px',
                      background: newSectionData.widthUnits === 3 ? 'var(--brand-primary)' : 'white',
                      color: newSectionData.widthUnits === 3 ? 'white' : 'var(--slate-700)',
                      border: `1px solid ${newSectionData.widthUnits === 3 ? 'var(--brand-primary)' : 'var(--slate-300)'}`,
                      borderRadius: '6px',
                      cursor: 'pointer',
                      fontSize: '13px',
                      fontWeight: 500,
                  ...
                  <button
                    type="button"
                    onClick={() => setNewSectionData(prev => ({ ...prev, widthUnits: 4 }))}
                    style={{
                      padding: '6px 12px',
                      background: newSectionData.widthUnits === 4 ? 'var(--brand-primary)' : 'white',
                      color: newSectionData.widthUnits === 4 ? 'white' : 'var(--slate-700)',
                      border: `1px solid ${newSectionData.widthUnits === 4 ? 'var(--brand-primary)' : 'var(--slate-300)'}`,
                      borderRadius: '6px',
                      cursor: 'pointer',
                      fontSize: '13px',
                      fontWeight: 500,
                  ...
                  <button
                    type="button"
                    onClick={() => setNewSectionData(prev => ({ ...prev, widthUnits: 6 }))}
                    style={{
                      padding: '6px 12px',
                      background: newSectionData.widthUnits === 6 ? 'var(--brand-primary)' : 'white',
                      color: newSectionData.widthUnits === 6 ? 'white' : 'var(--slate-700)',
                      border: `1px solid ${newSectionData.widthUnits === 6 ? 'var(--brand-primary)' : 'var(--slate-300)'}`,
                      borderRadius: '6px',
                      cursor: 'pointer',
                      fontSize: '13px',
                      fontWeight: 500,
                  ...
                  <button
                    type="button"
                    onClick={() => setNewSectionData(prev => ({ ...prev, widthUnits: 8 }))}
                    style={{
                      padding: '6px 12px',
                      background: newSectionData.widthUnits === 8 ? 'var(--brand-primary)' : 'white',
                      color: newSectionData.widthUnits === 8 ? 'white' : 'var(--slate-700)',
                      border: `1px solid ${newSectionData.widthUnits === 8 ? 'var(--brand-primary)' : 'var(--slate-300)'}`,
                      borderRadius: '6px',
                      cursor: 'pointer',
                      fontSize: '13px',
                      fontWeight: 500,
                  ...
                  <button
                    type="button"
                    onClick={() => setNewSectionData(prev => ({ ...prev, widthUnits: 12 }))}
                    style={{
                      padding: '6px 12px',
                      background: newSectionData.widthUnits === 12 ? 'var(--brand-primary)' : 'white',
                      color: newSectionData.widthUnits === 12 ? 'white' : 'var(--slate-700)',
                      border: `1px solid ${newSectionData.widthUnits === 12 ? 'var(--brand-primary)' : 'var(--slate-300)'}`,
                      borderRadius: '6px',
                      cursor: 'pointer',
                      fontSize: '13px',
                      fontWeight: 500,
                ...
                <input
                  type="number"
                  value={newSectionData.widthUnits}
                  onChange={(e) => {
                    const value = parseInt(e.target.value) || 12;
                    setNewSectionData(prev => ({ ...prev, widthUnits: Math.max(1, Math.min(12, value)) }));
                  }}
                  min="1"
                  max="12"
                  step="1"
                ...
                />
                <p style={{ margin: '6px 0 0 0', fontSize: '12px', color: 'var(--slate-500)' }}>
                  Select a preset or enter a custom value (1-12 columns)
                </p>
              </div>
            </div>
            <div className="modal-actions">
              <button className="btn-secondary" onClick={() => setShowAddSectionModal(false)}>Cancel</button>
              <button className="btn-primary" onClick={handleAddSection} disabled={!newSectionData.id || !newSectionData.title || !newSectionData.linkedTabId}>
...
function SuccessPlanningPage({ clientInfo, setClientInfo, onSave }) {
  ...
  return (
    <div className="admin-container">
      ...
      {/* FLOATING ACTION BUTTONS - Only shown when changes are made */}
      {hasUnsavedChanges && (
        <div
          style={{
            position: 'fixed',
...
function AdminPage({ formData, setFormData, stories, setStories, opportunities, setOpportunities, clientInfo, setClientInfo, onSave, onClientNameChange, isNewClient, onNewClientHandled, selectedClientId, selectedMonth, legacyStoriesData, onNavigateToStories, onNavigateToPriorities }) {
  const { hasPermission } = useAuth();
  const customizationContext = useCustomization();
  const { customization, getFormSchema, updateFormSchema, updateDeletedSectionIds } = customizationContext;
  const canEditSensitiveData = hasPermission('edit_sensitive_data');
  ...
  const [localClientInfo, setLocalClientInfo] = useState(clientInfo);
  const [localFormSchema, setLocalFormSchema] = useState(() => getFormSchema());
  const [localDeletedSectionIds, setLocalDeletedSectionIds] = useState(() => customization.deletedSectionIds || []);

  // Track if there are unsaved changes
  const hasUnsavedChanges = useMemo(() => {
    // Compare local form data with original
    ...
    const schemaChanged = JSON.stringify(localFormSchema) !== JSON.stringify(getFormSchema());
    const deletedSectionsChanged = JSON.stringify(localDeletedSectionIds) !== JSON.stringify(customization.deletedSectionIds || []);

    return formChanged || storiesChanged || opportunitiesChanged || clientInfoChanged || schemaChanged || deletedSectionsChanged;
  }, [localForm, formData, localStories, stories, localOpportunities, opportunities, localClientInfo, clientInfo, localFormSchema, getFormSchema, localDeletedSectionIds, customization.deletedSectionIds]);

  // Ensure active tab is always valid (switch to first enabled tab if current is disabled)
  useEffect(() => {
  ...
  useEffect(() => {
    setLocalFormSchema(getFormSchema());
  }, [customization.formSchema, getFormSchema]);

  useEffect(() => {
    setLocalDeletedSectionIds(customization.deletedSectionIds || []);
  }, [customization.deletedSectionIds]);
  ...
  const [sectionVisibility, setSectionVisibility] = useState(() => {
    ...
    return { ...defaults, ...formData.sectionVisibility };
  });
  ...
  const localUpdateFormField = (sectionId, fieldId, updates) => {
    setLocalFormSchema(prev => updateField(prev, sectionId, fieldId, updates));
  };
  ...
  const localAddFormSection = (section) => {
    setLocalFormSchema(prev => ({
      ...prev,
  ...
  const handleSave = () => {
    // Save form schema changes first
    if (JSON.stringify(localFormSchema) !== JSON.stringify(getFormSchema())) {
      // Apply schema changes to global state
      updateFormSchema(localFormSchema);
    }

    // Save deleted section IDs
        ...
        <>
          <SchemaFormRenderer
            ...
            localHandlers={{
              ...
              addFormSection: localAddFormSection,
              reorderFormSections: localReorderFormSections,
            }}
            clientId={clientInfo?.clientId || selectedClientId}
          />

      {/* Additional legacy sections for specific tabs with unique structures */}
      {activeTab === 'metrics' && (
              ...
              <div className="items-list">
                ...
                {localStories.filter(story => story.is_visible !== false).length === 0 && (
                  <div style={{ padding: '20px', textAlign: 'center', color: '#64748b', fontSize: '14px' }}>
                    No visible stories. Mark stories as visible in Success Planning.
                  </div>
                )}
                <button className="add-item-btn" onClick={onNavigateToStories}>+ Add/Edit Stories</button>
              </div>
            ) : (
              <div className="section-disabled-notice"><EyeOff size={16} /> This section is hidden from reports</div>
            )}
...
function PrysmCSDashboardContent() {
  ...
  const setActivePage = (newPage) => {
    if (newPage !== activePage) {
      ...
      // Reset Success Planning tab to overview when navigating away
  ...
  const renderPage = () => {
    ...
    console.log('[renderPage] activePage:', activePage);
    console.log('[renderPage] available tabs:', customization.navigation.tabs);

    // Check if the active page is a custom tab
    const currentTab = customization.navigation.tabs.find(tab => tab.id === activePage);
    console.log('[renderPage] currentTab:', currentTab);

    if (currentTab?.isCustom) {
      console.log('[renderPage] Rendering CustomTabPage for:', currentTab.id);
      return <CustomTabPage tabId={currentTab.id} tabLabel={currentTab.label} data={dashboardData} sectionVisibility={sectionVisibility} />;
    }

    switch (activePage) {
      case "overview":
        return hasPermission('view_dashboard') ? <OverviewPage data={dashboardData} sectionVisibility={sectionVisibility} brandColor={customization.branding.primaryColor} clientId={selectedClientId} /> : <AccessDeniedMessage />;
      case "enrollment":
      ...
      case "successPlanning":
        console.log('[renderPage] Rendering Success Planning with tab:', successPlanningTab);
        ...
        return hasPermission('edit_data') ? (
          <AdminPage
            ...
            onNavigateToPriorities={() => {
              console.log('[onNavigateToPriorities] Navigating to Success Planning - Plan tab');
      ...
      default: return <OverviewPage data={dashboardData} sectionVisibility={sectionVisibility} clientId={selectedClientId} />;
    }
  };
