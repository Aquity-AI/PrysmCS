...
function generateDefaultSlides(dashboardData, clientInfo, stories, opportunities, monthLabel, branding = {}, schemaVisibility = {}) {
  ...
  const visibility = {
    coreMetrics: schemaVisibility.coreMetrics ?? true,
    enrollmentFunnel: schemaVisibility.enrollmentFunnel ?? true,
    campaignPerformance: schemaVisibility.campaignPerformance ?? true,
    patientOutcomes: schemaVisibility.patientOutcomes ?? true,
    stories: schemaVisibility.stories ?? true,
    opportunities: schemaVisibility.opportunities ?? true,
  };
...
function PresentationMode({ isOpen, onClose, dashboardData, clientInfo, stories, opportunities, monthLabel, formData }) {
  ...
  // Get section visibility from schema
  const schemaVisibility = {
    coreMetrics: formSchema.sections.find(s => s.id === 'coreMetrics')?.enabled === true,
    enrollmentFunnel: formSchema.sections.find(s => s.id === 'enrollmentFunnel')?.enabled === true,
    campaignPerformance: formSchema.sections.find(s => s.id === 'smsCampaign')?.enabled === true ||
                         formSchema.sections.find(s => s.id === 'emailCampaign')?.enabled === true ||
                         formSchema.sections.find(s => s.id === 'mailerCampaign')?.enabled === true,
    patientOutcomes: formSchema.sections.find(s => s.id === 'patientOutcomes')?.enabled === true,
    stories: true,
    opportunities: true,
  };
  ...
  useEffect(() => {
    if (isOpen && !wasOpenRef.current) {
      ...
      setSlides(generateDefaultSlides(dashboardData, clientInfo, stories, opportunities, monthLabel, customization.branding, schemaVisibility));
      setCurrentSlide(0);
  ...
  }, [isOpen, dashboardData, clientInfo, stories, opportunities, monthLabel, customization.branding, formSchema]);
        ...
        <div className="pres-slide-enrollment-layout">
          ...
          <div className="pres-enrollment-grid">
            <div className="pres-funnel-section">
              <h3>{enrollmentFunnelSection?.title || 'Customer Funnel'}</h3>
